...
def compare_sentences(sentences,model,tokenizer):
    ...
    print(sentences[0], '|', sentences[1],": ", similarity)
    return similarity
...
if __name__ == "__main__":
    ...
    filename = input("Enter file name to do in batch mode: ")
    sentences = []
    ...
    wfp = open("debug_" + filename.split('.')[0] + ".txt","w")
    ...
    for i in range(len(sentences)):
              ...
              for j in range(len(sentences)): 
                   ...
                   pass_sentences.append(sentences[j])
                   score = compare_sentences(pass_sentences,model,tokenizer)
                   #wfp.write(pass_sentences[0] + '|' + pass_sentences[1] + ": " + str(score) + "\n")
                   sent_dict[pass_sentences[0] + '|' + pass_sentences[1]] = score 
                   similarity_matrix[i][j] = score
              sorted_d  = OrderedDict(sorted(sent_dict.items(), key=lambda kv: kv[1], reverse=True))
              for sent in sorted_d:
                   wfp.write(sent + ": " + str(sorted_d[sent]) + "\n") 
    output_file = "matrix_" + filename.split('.')[0] + ".npy"
    with open(output_file,"wb") as mfp:
         np.save(mfp,similarity_matrix)
    wfp.close()
